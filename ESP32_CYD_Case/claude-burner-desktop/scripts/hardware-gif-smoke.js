...
const scenes = requested.length
  ? requested
  : assets.catalog.states.map(({ key }) => key);
const holdMilliseconds = Number(process.env.BURNER_GIF_HOLD_MS || 5000);
const expectedFps = assets.fps;
const frameBudgetMicros = Math.round(1_000_000 / expectedFps);
...
async function main() {
  ...
  try {
    ...
    for (const scene of scenes) {
      const state = assets.catalog.states.find(({ key }) => key === scene);
      if (!state) throw new Error(`Unknown active catalog scene: ${scene}`);
      const animation = assets.getDeviceGif(scene);
      const life = state.level === 1 ? 25 : state.level === 2 ? 50 : 100;
      const install = await serial.installGif(animation.bytes, animation.checksum, life, state.level);
      const playback = await observePlayback(serial, holdMilliseconds);
      const cached = await serial.installGif(animation.bytes, animation.checksum, life, state.level);
      ...
      if (playback.observedFps < expectedFps * 0.96) {
        throw new Error(`${scene} missed the physical ${expectedFps} FPS target (${playback.observedFps.toFixed(2)} FPS)`);
      }
      if (playback.lastFrameRenderMicros > frameBudgetMicros) {
        throw new Error(`${scene} exceeded the ${frameBudgetMicros} us frame budget (${playback.lastFrameRenderMicros} us)`);
      }
    ...
    const report = { devicePath, protocol: 2, expectedFps, frameBudgetMicros, results };
