...
test("generation wait explains durable staged progress without quota noise", async () => {
  ...
  assert.match(workspace, /<h1 id="workspace-title">\{stage.label\}<\/h1>/);
  assert.match(workspace, /buildId \? "You can leave this tab. We’ll keep working."/);
  assert.match(workspace, /Build generation progress/);
  assert.match(workspace, /mk-generation-original"><p>\{prompt\}<\/p>/);
...
test("the generated project journey is overview with parts, wiring, then locked code", async () => {
  ...
  assert.match(workspace, /disabled=\{!wiringAvailable \|\| wiringLoading\}/);
  ...
  assert.match(wiringGuide, /<SavedWiringViewer key=\{build.id\} buildId=\{build.id\} stepIndex=\{boundedIndex\}/);
...
test("safe recovery preserves identity and exposes every supported exit", async () => {
  ...
  assert.match(workspace, /<summary>Build details<\/summary>/);
  assert.match(workspace, /Edit my idea/);
  assert.match(workspace, /backLabel = "My projects"/);
