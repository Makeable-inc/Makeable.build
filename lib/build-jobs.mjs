...
export async function getDraftJobImage({ request, stateStore, imageStore, env = {}, jobId, user = null } = {}) {
  const access = await resolveBuildJobAccess({ request, stateStore, env, jobId, user });
  if (!access.ok) return access;
