...
import {
  presentationImageForBuild,
  reconcileBuildPartsWithAssembly,
} from "./makeable-builds.mjs";
...
const PIPELINE_PHASE_TO_JOB_STATE = Object.freeze({
  planning: BUILD_JOB_STATES.planning,
  fitting_parts: BUILD_JOB_STATES.fittingParts,
  assembling: BUILD_JOB_STATES.fittingParts,
  rendering: BUILD_JOB_STATES.rendering,
});

export function buildJobStateForPipelinePhase(phase) {
  const state = PIPELINE_PHASE_TO_JOB_STATE[String(phase || "")];
  if (!state) throw new Error("Invalid build pipeline phase");
  return state;
}
...
const CLIENT_REQUEST_ID_PATTERN = /^req_[A-Za-z0-9_-]{16,136}$/;
const COOKIE_VALUE_PATTERN = /^v1\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/;
const BACKGROUND_SIGNATURE_TOLERANCE_MS = 5 * 60 * 1000;
// Production deploy 6a9a34c186424ed4f2994352 made the atomic manifest,
// exact BOM, and saved wiring guide mandatory. Deploy URLs are immutable and
// Netlify Blobs are site-scoped, so an older still-open deploy can otherwise
// write a legacy Concept record into the current production stores. Treat the
// cutover as a data boundary, not merely a property supplied by the caller.
export const ATOMIC_BUILD_REQUIRED_SINCE = "2026-09-04T03:02:35.753Z";
const KNOWN_UNUSABLE_BUILD_IDS = new Set([
  "build-a-desktop-indoor-comfort-station-tha-4017c5a0",
]);
...
export function builderDisabled(env, operation) {
  // During an atomic cutover production sets the legacy global switch to 0 and
  // this epoch-specific switch to 1. Older immutable deploys only understand
  // the global switch and stop generating; current code is explicitly allowed.
  const currentAtomicEpochEnabled = explicitlyEnabledEnv(
    env.MAKEABLE_ATOMIC_BUILD_GENERATION_ENABLED,
  );
  if (!currentAtomicEpochEnabled && !enabledEnv(env.MAKEABLE_BUILD_GENERATION_ENABLED)) {
...
export async function createAnonymousBuildJob({
  ...
  const accountHash = authenticatedSub
    ...
    : "";
  const suppliedRequestId = String(clientRequestId || "").trim();
  if (suppliedRequestId && !CLIENT_REQUEST_ID_PATTERN.test(suppliedRequestId)) {
    return failure("Build request id is invalid.", 400);
  }
  const requestId = suppliedRequestId || `req_${randomToken(randomBytesImpl, 16)}`;
  const requestHash = scopedHash(
    secret,
    `request:${accountHash || browserHash}:${requestId}`,
  );
  const existingRequestJob = await buildForRequestIndex(
    stateStore,
    requestHash,
    timestamp,
  );
  if (existingRequestJob) {
    if (existingRequestJob.idea !== normalizedIdea) {
      return failure("That build request id is already attached to a different idea.", 409);
    }
    return duplicateBuildRequest(existingRequestJob, authenticatedSub);
  }

  // Signed-in jobs may run in parallel. Browser/IP active indexes are only an
  // anonymous abuse-control boundary; exact retries use the request index above.
  const activeIndexes = accountHash
    ? []
    : [
  ...
  const active = activeIndexes.length
    ? await activeBuildForIndexes(stateStore, activeIndexes, timestamp)
    : null;
  if (active) {
    return {
      ok: false,
  ...
  const startCounts = authenticatedSub
    ? [await recentAccountCountedStartCount(stateStore, accountHash, timestamp)]
    : await Promise.all([
  ...
  const buildId = `build_${slugToken(normalizedIdea)}_${randomToken(randomBytesImpl, 10)}`;
  const identity = createBuildRequestIdentity({
    requestId,
    jobId,
    buildId,
    prompt: normalizedIdea,
    accountHash,
    catalogRevision: revisionContext?.catalogRevision || "legacy-catalog",
    promptPackageRevision: revisionContext?.promptPackageRevision || "legacy-prompts",
  });
  ...
  const job = {
    ...
    ipHash,
    accountHash,
    ...(authenticatedSub ? { ownerSub: authenticatedSub } : {}),
    identity,
    completionPolicy: {
      requireAtomicManifest: revisionContext?.requireAtomicManifest === true,
    },
  ...
  const requestClaim = await claimBuildRequestIndex({
    stateStore,
    requestHash,
    job,
    timestamp,
  ...
  if (!requestClaim.ok) {
    await deleteQuietly(stateStore, jobKey(jobId));
    if (requestClaim.job?.idea !== normalizedIdea) {
      return failure("That build request id is already attached to a different idea.", 409);
    }
    return duplicateBuildRequest(requestClaim.job, authenticatedSub);
  }

  const indexWrites = accountHash
    ? [
...
export async function getActiveBuildJobForRequest({
  request,
  stateStore,
  env = {},
  requestId: clientRequestId = "",
  ...
  const secret = draftCookieSecret(env);
  if (!secret) return failure("Build draft signing is not configured.", 503);
  ...
  if (suppliedRequestId && (accountHash || cookie.state === "valid")) {
    const browserHash = cookie.state === "valid"
      ? scopedHash(secret, `browser:${cookie.payload.browserId}`)
      : "";
    const requestHash = scopedHash(
      secret,
      `request:${accountHash || browserHash}:${suppliedRequestId}`,
    );
    const exact = await buildForRequestIndex(stateStore, requestHash, timestamp);
    if (exact) return { ok: true, job: publicBuildJob(exact) };
  }
  // Never perform a broad account lookup: more than one signed-in build may be
  // active, and returning an arbitrary one would change the user's project.
  if (accountHash) return failure("No active build job was found for that request.", 404);

  const activeIndexes = [
      ...(cookie.state === "valid"
        ? [{
          kind: "browser",
          hash: scopedHash(secret, `browser:${cookie.payload.browserId}`),
        }]
        : []),
      {
        kind: "ip",
        hash: scopedHash(secret, `ip:${clientIpFromRequest(request) || "unknown"}`),
      },
    ];

  const active = await activeBuildForIndexes(stateStore, activeIndexes, timestamp);
  if (!active) return failure("No active build job was found.", 404);
  return { ok: true, job: publicBuildJob(active) };
}
...
export async function completeBuildJob({
  ...
  const completedMetadata = buildJobRequiresAtomicManifest(job)
    ? bindAtomicBuildManifest(metadata, job.identity)
    : metadata;
  const updated = {
    ...job,
...
export async function failBuildJob(stateStore, jobId, error, now = new Date()) {
  const job = await getBuildJob(stateStore, jobId);
  if (!job || TERMINAL_STATES.has(job.state)) return job;
  const failure = cleanBuildFailure(error);
  const updated = {
    ...job,
...
export async function claimSuccessfulBuildJob({
  ...
  if (buildJobRequiresAtomicManifest(job)) {
    try {
      assertAtomicBuildManifest(job.result, job.identity);
    } catch (error) {
      return failure(`This build is not usable and no credit was charged (${error.message}).`, 422);
    }
  }
  ...
  try {
    ...
    if (buildJobRequiresAtomicManifest(job)) {
      const durable = await getAccountGalleryBuild(stateStore, buildId, normalizedUser.sub);
      assertAtomicBuildManifest(durable, job.identity);
    }
...
export async function resolveBuildJobAccess({
  ...
  const normalizedUser = normalizeAccountUser(user);
  if (normalizedUser?.sub && buildJobBelongsToAccount(job, normalizedUser.sub, env)) {
    return { ok: true, job, via: "owner" };
  }
  if (normalizedUser?.sub && (job.ownerSub || job.accountHash)) {
    return failure("This build belongs to another Google account.", 403);
  }
...
export async function listAccountGalleryBuilds(stateStore, sub) {
  ...
  return records
    .filter((record) => record?.ownerSub === sub && record?.creditDisposition?.state !== "refunded")
...
/**
 * Idempotently refund unusable claimed builds. The named historical incident
 * remains covered, and every post-cutover build is now required to carry the
 * same atomic overview/BOM/wiring contract. This catches writes from stale
 * deploy URLs without deleting the older community catalog.
 */
export async function refundKnownUnusableBuildCredit(stateStore, sub, now = new Date()) {
  if (!sub) return { refunded: false, buildIds: [] };
  const refundedBuildIds = [];
  const records = await listJsonBlobs(stateStore, "gallery/");
  const candidates = records.filter((record) => (
    record?.ownerSub === sub
    && record?.creditDisposition?.state !== "refunded"
    && (KNOWN_UNUSABLE_BUILD_IDS.has(record.id) || postCutoverBuildIsUnusable(record))
  ));
  for (const record of candidates) {
    const buildId = record.id;
    if (!record || record.ownerSub !== sub || record.creditDisposition?.state === "refunded") continue;
    const slots = await listJsonBlobEntries(stateStore, `${quotaPrefix(sub)}/`);
    const chargedSlot = slots.find(({ value }) => (
      value?.sub === sub && value?.jobId === record.sourceJobId && value?.status === "used"
    ));
    if (chargedSlot) await deleteQuietly(stateStore, chargedSlot.key);
    const refundedAt = validDate(now).toISOString();
    await setJsonBlob(stateStore, galleryKey(buildId), {
      ...record,
      publishState: "hidden",
      hiddenAt: record.hiddenAt || refundedAt,
      updatedAt: refundedAt,
      creditDisposition: {
        state: "refunded",
        reason: postCutoverBuildIsUnusable(record)
          ? "post_cutover_atomic_wiring_contract_missing"
          : "atomic_manifest_or_exact_route_unavailable",
        refundedAt,
      },
    });
    refundedBuildIds.push(buildId);
  }
  return { refunded: refundedBuildIds.length > 0, buildIds: refundedBuildIds };
}

function buildJobRequiresAtomicManifest(job) {
  return job?.completionPolicy?.requireAtomicManifest === true
    || timestampAtOrAfterAtomicCutover(job?.createdAt);
}

function postCutoverBuildIsUnusable(record) {
  if (!timestampAtOrAfterAtomicCutover(record?.claimedAt || record?.createdAt)) return false;
  try {
    assertAtomicBuildManifest(record, record?.identity);
    return false;
  } catch {
    return true;
  }
...
function timestampAtOrAfterAtomicCutover(value) {
  const timestamp = Date.parse(String(value || ""));
  return Number.isFinite(timestamp) && timestamp >= Date.parse(ATOMIC_BUILD_REQUIRED_SINCE);
}

export async function getPublicGalleryBuild(stateStore, buildId) {
  if (!SAFE_ID_PATTERN.test(String(buildId || ""))) return null;
  return publicGalleryBuildDetail(await getJsonBlob(stateStore, galleryKey(buildId)))
    || publicGalleryBuildDetail(await getJsonBlob(stateStore, legacyGalleryKey(buildId)));
}
...
export async function getDraftJobImage({
  request,
  stateStore,
  imageStore,
  env = {},
...
export function publicBuildJob(job) {
  ...
  return {
    ...
    buildId: job.buildId || "",
    identity: publicBuildIdentity(job.identity),
    ...
    failure: job.state === BUILD_JOB_STATES.failed ? cleanBuildFailure(job.failure || job.error) : null,
...
function publicGalleryBuildDetail(record) {
  const shaped = publicGalleryBuild(record);
  if (!shaped) return null;
  const assembly = record.artifacts?.assembly;
  if (!assembly) return shaped;
  return {
    ...shaped,
    ...(record.semanticFulfillment ? {
      semanticFulfillment: {
        ok: record.semanticFulfillment.ok === true,
        coveragePercent: Number(record.semanticFulfillment.coveragePercent || 0),
      },
    } : {}),
    artifacts: {
      assembly,
      delivery: {
        modelFetches: Array.isArray(record.artifacts?.delivery?.modelFetches)
          ? record.artifacts.delivery.modelFetches
          : [],
      },
      wiring: {
        standard: String(record.artifacts?.wiring?.standard || ""),
      },
    },
  };
}
...
export function accountGalleryBuild(record) {
  ...
  return {
    ...shaped,
    // Preserve the owner's original request so the project folder can compile
    // its Wiring view from the exact intent instead of reverse-engineering a
    // prompt from the generated title or marketing summary. Public gallery
    // responses intentionally continue to omit this private source text.
    idea: typeof record.idea === "string" ? record.idea : "",
    image: presentationImageForBuild(record) || shaped.image,
    parts: reconcileBuildPartsWithAssembly(record.parts, record.artifacts),
    ...
    ...(record.artifactStates ? { artifactStates: record.artifactStates } : {}),
    ...(record.semanticFulfillment ? { semanticFulfillment: record.semanticFulfillment } : {}),
...
function shapeGalleryBuild(record, overrides = {}) {
  ...
  const publicManifest = record.manifest ? {
    schemaVersion: record.manifest.schemaVersion,
    identity: publicBuildIdentity(record.manifest.identity),
    manifestSha256: record.manifest.manifestSha256,
  } : null;
  const publicAssembly = record.artifacts?.assembly ? {
    schemaVersion: record.artifacts.assembly.schemaVersion,
    state: record.artifacts.assembly.state,
  } : null;
  const reconciledParts = reconcileBuildPartsWithAssembly(record.parts, record.artifacts);
  return {
    id: record.id,
    ...
    visibleHardwareCues: Array.isArray(record.visibleHardwareCues)
      ...
      : [],
    image: publicImage(presentationImageForBuild(record)),
    parts: reconciledParts.map(publicPart).filter(Boolean),
    warnings: Array.isArray(record.warnings) ? record.warnings : [],
    cost: record.cost || null,
    status: record.status || "Concept",
    ...(record.identity ? { identity: publicBuildIdentity(record.identity) } : {}),
    ...(publicManifest ? { manifest: publicManifest } : {}),
    ...(record.artifactStates ? { artifactStates: record.artifactStates } : {}),
    ...(publicAssembly ? { artifacts: { assembly: publicAssembly } } : {}),
  };
}
...
function publicPart(part) {
  ...
  const sensorSpec = sensorSpecForPart(part);
  const allowed = [
    "id",
    "name",
    "category",
  ...
  if (thumbnailUrl) safe.thumbnailUrl = thumbnailUrl;
  else delete safe.thumbnailUrl;
  if (sensorSpec) safe.sensorSpec = sensorSpec;
  return Object.keys(safe).length ? safe : null;
}
  ...
  return {
    metadata: {
      ...
      image: {
        ...
        url: imageUrl,
        contentType: match[1],
        contentSha256: createHash("sha256").update(Buffer.from(match[2], "base64")).digest("hex"),
      },
    },
    imageBytes: {
...
function duplicateBuildRequest(job, authenticatedSub = "") {
  return {
    ok: false,
    status: 409,
    error: authenticatedSub
      ? "This exact build request is already in the workshop."
      : "This exact build request is already active in this browser.",
    activeJob: publicBuildJob(job),
    headers: { "Retry-After": "1" },
  };
}

async function claimBuildRequestIndex({ stateStore, requestHash, job, timestamp }) {
  const key = activeKey("request", requestHash);
  const expiresAt = new Date(timestamp.getTime() + BUILD_JOB_LIMITS.startWindowMs).toISOString();
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    if (write?.modified !== false) return { ok: true, job };
    const existing = await buildForRequestIndex(stateStore, requestHash, timestamp);
    if (existing) return { ok: false, job: existing };
  }
  return { ok: false, job: null };
}

async function buildForRequestIndex(stateStore, requestHash, now) {
  const key = activeKey("request", requestHash);
  const pointer = await getJsonBlob(stateStore, key);
  if (!pointer?.jobId) return null;
  const expired = validTimestamp(pointer.expiresAt) <= now;
  const job = expired ? null : await getBuildJob(stateStore, pointer.jobId);
  if (!job) {
    await deleteQuietly(stateStore, key);
    return null;
  }
...
async function activeBuildForIndexes(stateStore, indexes, now) {
  for (const { kind, hash } of indexes) {
    ...
    if (!active) continue;
    // Do not return an authenticated job through an anonymous browser/IP
    // index, including legacy indexes written by older deployments.
    if (!active.accountHash) return active;
  }
  return null;
}
...
async function clearActiveIndexes(stateStore, job) {
  const keys = [
    clearActiveIndexIfOwned(stateStore, "browser", job.browserHash, job.id),
    clearActiveIndexIfOwned(stateStore, "ip", job.ipHash, job.id),
  ];
  if (job.accountHash) {
    keys.push(clearActiveIndexIfOwned(stateStore, "account", job.accountHash, job.id));
  }
  await Promise.all(keys);
}

async function clearActiveIndexIfOwned(stateStore, kind, hash, jobId) {
  if (!hash) return;
  const key = activeKey(kind, hash);
  const active = await getJsonBlob(stateStore, key);
  if (active?.jobId === jobId) await deleteQuietly(stateStore, key);
}

function buildJobBelongsToAccount(job, sub, env) {
  const normalizedSub = typeof sub === "string" ? sub.trim() : "";
  if (!normalizedSub) return false;
  if (job?.ownerSub) return job.ownerSub === normalizedSub;
  const secret = draftCookieSecret(env);
  if (!secret || !job?.accountHash) return false;
  return job.accountHash === scopedHash(secret, `account:${normalizedSub}`);
}
...
async function recentAccountCountedStartCount(stateStore, hash, now) {
  const events = await listJsonBlobs(stateStore, `starts/account/${hash}/`);
  const minTime = now.getTime() - BUILD_JOB_LIMITS.startWindowMs;
  const recent = events.filter((event) => validTimestamp(event?.createdAt).getTime() >= minTime);
  const jobs = await Promise.all(recent.map((event) => getBuildJob(stateStore, event?.jobId)));
  return jobs.filter((job) => (
    !job || !new Set([BUILD_JOB_STATES.failed, BUILD_JOB_STATES.cancelled]).has(job.state)
  )).length;
}
...
function cleanBuildFailure(error) {
  ...
  const failure = {
    ...
    code: cleanFailureCode(source.code),
    details: {},
  };
  for (const key of ["reason", "plannerTitle"]) {
    const value = cleanFailureText(details[key], key === "plannerTitle" ? 120 : 180);
    if (value) failure.details[key] = value;
  }
  for (const key of [
    "plannerWarnings",
    ...
    const values = cleanFailureList(details[key]);
    if (values.length) failure.details[key] = values;
  }
  return failure;
}
...
function cleanFailureText(value, maxLength = 180) {
  return typeof value === "string"
    ? value.replace(/\s+/g, " ").trim().slice(0, maxLength)
    : "";
}

function cleanFailureList(value) {
  return Array.isArray(value)
    ? value.map((entry) => cleanFailureText(entry, 180)).filter(Boolean).slice(0, 8)
    : [];
}
