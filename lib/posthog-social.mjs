...
const SOCIAL_WEBSITE_SESSIONS_QUERY = `
  SELECT
    toString(lastTouch.1) AS date,
    lastTouch.2 AS platform,
    lastTouch.3 AS accountKey,
    count() AS websiteSessions
  FROM (
    SELECT
      properties['$session_id'] AS sessionId,
      argMax(
        tuple(
          toDate(timestamp),
          properties['social_platform'],
          properties['social_account']
        ),
        timestamp
      ) AS lastTouch
    FROM events
...
export async function readSocialWebsiteSessions(options = {}) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      if (!response || response.ok !== true || typeof response.json !== "function") continue;
      const daily = normalizeRows(await response.json());
      if (daily) return { status: "connected", daily };
    } catch {
      // One immediate retry protects the dashboard from short-lived analytics failures.
    }
  }
  return UNAVAILABLE;
}
...
function normalizeRows(payload) {
  ...
  if (payload.results.length > MAX_RESULT_ROWS) return null;
