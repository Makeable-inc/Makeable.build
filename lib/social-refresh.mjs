...
async function refreshTikTokRecords({ accessToken, fetchImpl }) {
  const url = queryUrl(`${TIKTOK_API}/video/list/`, { fields: "id,title,video_description,create_time,cover_image_url,share_url,embed_link,view_count,like_count,comment_count,share_count" });
  const body = await jsonRequest(url, "tiktok", fetchImpl, { Authorization: `Bearer ${accessToken}` });
  return requireRecords("tiktok", (Array.isArray(body?.data?.videos) ? body.data.videos : []).map(normalizeTikTokRecord).filter(Boolean));
}
...
function normalizeTikTokRecord(record) {
...
function socialRecord({ platform, account, contentId, publishedAt, contentType, caption, impressions, engagements, attributionKey, thumbnailUrl, postUrl, coverage = "connected", engagementsComplete = false }) {
  return { id: `${platform}:${account}:${contentId}`, platform, account, publishedAt, contentId, contentType, caption: caption.slice(0, 500), impressions, engagements, followers: 0, followersGained: null, clicks: null, coverage, attributionKey, engagementsComplete, thumbnailUrl, previewUrl: "", postUrl };
}
