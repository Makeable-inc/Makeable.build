...
function normalizeYouTubeRecord(video, owner = null) {
  ...
  return socialRecord({
    ...
    embedUrl: `https://www.youtube.com/embed/${encodeURIComponent(contentId)}`,
...
function normalizeTikTokRecord(record, followers) {
  ...
  return socialRecord({ platform: "tiktok", account: "@trymakeable.build", contentId, publishedAt, contentType: "video", caption: string(record?.title || record?.video_description), impressions: number(record?.view_count), engagements: number(record?.like_count) + number(record?.comment_count) + number(record?.share_count), followers, attributionKey: "trymakeable_build", thumbnailUrl: safeUrl(record?.cover_image_url), embedUrl: safeUrl(record?.embed_link), postUrl: safeUrl(record?.share_url || record?.embed_link), engagementsComplete: true });
}

function socialRecord({ platform, account, contentId, publishedAt, contentType, caption, impressions, engagements, followers = 0, followersGained = null, clicks = null, attributionKey, thumbnailUrl, embedUrl = "", postUrl, coverage = "connected", engagementsComplete = false, analytics = null }) {
  return { id: `${platform}:${account}:${contentId}`, platform, account, publishedAt, contentId, contentType, caption: caption.slice(0, 500), impressions, engagements, followers, followersGained, clicks, coverage, attributionKey, engagementsComplete, thumbnailUrl, previewUrl: "", ...(embedUrl ? { embedUrl } : {}), postUrl, ...(analytics ? { analytics } : {}) };
}
