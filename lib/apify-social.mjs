...
const ACCOUNTS = Object.freeze([
  ...
  },
  {
    ...
    input: {
      startUrls: [{ url: "https://www.youtube.com/@makeablebuild" }],
      maxResults: 12,
      maxResultsShorts: 12,
      maxResultStreams: 0,
    },
...
function normalizeRecord(platform, row) {
  ...
  const publishedAt = timestamp(platform === "instagram" ? row.timestamp
    : platform === "facebook" ? row.publishedAt || row.timestampMs || row.timestamp || row.createdAt
  ...
  const impressions = number(platform === "instagram" ? row.videoPlayCount || row.videoViewCount
    : platform === "facebook" ? row.plays || row.views || row.viewsCount || row.viewCount
  ...
  return {
    ...
    caption: string(platform === "instagram" ? row.caption : platform === "youtube" ? row.title || row.text : row.text).slice(0, 500),
    ...
    thumbnailUrl: safeUrl(platform === "instagram" ? row.displayUrl
      : platform === "facebook" ? row.thumbnailUrl || row.videoThumbnailUrl || row.images?.[0]
    ...
    postUrl: safeUrl(platform === "instagram" ? row.url : platform === "facebook" ? row.contentUrl || row.postUrl
...
function accountFor(platform, row) {
  ...
  if (platform === "youtube" && string(row.channelUrl || row.channelHandle || row.author).toLowerCase().includes("makeablebuild")) return "@makeablebuild";
  return "";
}
