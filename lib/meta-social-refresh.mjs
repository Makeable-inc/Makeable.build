...
function normalizeInstagramRecord(record, account, attributionKey, followers, accountAnalytics) {
  ...
  return socialRecord({ platform: "instagram", account, contentId, publishedAt, contentType: string(record?.media_product_type || record?.media_type || "post").toLowerCase(), caption: string(record?.caption), impressions: insight(record?.insights, ["views", "impressions", "reach"]), engagements: totalInteractions || number(record?.like_count) + number(record?.comments_count) + insight(record?.insights, ["shares", "saved"]), followers, accountAnalytics, attributionKey, thumbnailUrl: safeUrl(record?.thumbnail_url || record?.media_url), postUrl: permalink });
}
...
function normalizeFacebookRecord(record, followers, accountAnalytics) {
  ...
  if (!contentId || !publishedAt) return null;
  return socialRecord({ platform: "facebook", account: "Makeable Facebook", contentId, publishedAt, contentType: "post", caption: string(record?.message), impressions: insight(record?.insights, ["post_media_view"]), engagements: number(record?.reactions?.summary?.total_count) + number(record?.comments?.summary?.total_count) + number(record?.shares?.count), followers, clicks: insight(record?.insights, ["post_clicks"]), accountAnalytics, attributionKey: "makeable_facebook", thumbnailUrl: safeUrl(record?.full_picture), postUrl: safeUrl(record?.permalink_url) });
}
...
function socialRecord({ platform, account, contentId, publishedAt, contentType, caption, impressions, engagements, followers, clicks = null, accountAnalytics = null, attributionKey, thumbnailUrl, postUrl }) {
  return { id: `${platform}:${account}:${contentId}`, platform, account, publishedAt, contentId, contentType, caption: caption.slice(0, 500), impressions, engagements, followers, followersGained: null, clicks, coverage: "connected", attributionKey, engagementsComplete: true, accountAnalytics, thumbnailUrl, previewUrl: "", postUrl };
}
