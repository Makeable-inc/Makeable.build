const CAPABILITY_RULES = Object.freeze([
  ...
  rule("touch_input", /\b(?:touch(?:screen|pad|[ -](?:screen|display|sensor|button|input|pad|activated|controlled))|capacitive (?:button|pad|touch)|tap[ -]activated|tap(?:ping)?\s+to\s+(?:change|cycle|switch|toggle|start|stop)|(?:each|every)\s+tap\s+(?:changes?|cycles?|switches?|toggles?))\b/i,
...
function hasAffirmativeMatch(text, pattern) {
  ...
  for (const match of text.matchAll(matcher)) {
    ...
    // Keep negation across lists ("no screen or buzzer"), but start a new
    // positive clause at "with/using/include". Commas alone do not end lists.
    // Process in order so a later negative can override a positive clause.
    const cues = [...localPrefix.matchAll(/\b(?:do\s+not|don't|never|without|exclude|excluding|excluded|skip|skipping|omit|omitting|ignore|ignoring|forbid|forbidden|disable|disabled|must\s+not|should\s+not|cannot|can't|no(?!\s+(?:matter|longer)\b)|with|using|include|includes|including|add|adds|just)\b/gi)];
    let negated = false;
    for (const cue of cues) {
      const positive = /^(?:with|using|include|includes|including|add|adds|just)$/i.test(cue[0]);
      if (!positive) negated = true;
      else if (/(?:,|\band)\s*$/i.test(localPrefix.slice(0, cue.index))) negated = false;
      // "without using" and "do not include" stay negative; only a new
      // affirmative clause can end a previously negative list.
    }
