...
export async function createPrompt2CircuitArtifacts({
  ...
  try {
    ...
    if (error?.code === "sol_part_set_not_acknowledged") {
      // Reject the entire model-written presentation. The circuit was already
      // compiled and validated above; derive a fresh presentation from that
      // exact contract rather than trusting or patching mismatched model IDs.
      presentation = {
        ...deterministicPresentation(placement),
        model: "deterministic-contract-renderer",
        reasoningEffort: "none",
      ...
      await onEvent("wiring_presentation_recovered", {
        reason: error.code,
        recovery: "discard-mismatched-presentation",
        ...error.details,
      });
    } else {
      // Style is descriptive metadata, never the source of physical routes.
      // Recover only this field after all identity/part/step/loop checks passed.
      if (error?.code !== "sol_ready_presentation_style_invalid") throw error;
      const rejectedStyle = error.details.style;
      presentation = {
        ...presentation,
        routingPresentation: { ...presentation.routingPresentation, style: "short-open-natural-arch" },
      };
      validatePresentationAcknowledgement(presentation, placement);
      await onEvent("wiring_presentation_recovered", {
        reason: error.code,
        rejectedStyle,
        recovery: "canonical-routing-style",
      });
    }
  }
...
export async function requestSolAssemblyPresentation({ env, fetchFn = fetch, prompt, placement, graph, resolved }) {
  ...
  const payload = {
    ...
    text: { format: { type: "json_schema", name: "makeable_assembly_presentation", strict: true, schema: presentationSchema(placement) } }
  };
...
function validatePresentationAcknowledgement(presentation, placement) {
  ...
  const received = Array.isArray(presentation.acknowledgedPartIds)
    ? [...presentation.acknowledgedPartIds].sort() : [];
  ...
  // Keep every other acknowledgement/safety check strict before considering
  // deterministic recovery for the model's descriptive part-list mismatch.
  if (JSON.stringify(expected) !== JSON.stringify(received)) {
    throw Object.assign(new Error("sol_part_set_not_acknowledged"), {
      code: "sol_part_set_not_acknowledged",
      details: {
        stage: "assembly-presentation",
        expectedPartIds: expected,
        receivedPartIds: received.filter(id => typeof id === "string").map(id => id.slice(0, 160)).slice(0, 40),
      },
    });
  }
  if (/\b(?:loop|coil|rectangular|manhattan|plumbing|pipe)\b/i.test(String(presentation.routingPresentation.style || ""))) {
    throw Object.assign(new Error("sol_ready_presentation_style_invalid"), {
      code: "sol_ready_presentation_style_invalid",
      details: { stage: "assembly-presentation", style: String(presentation.routingPresentation.style).slice(0, 240) },
    });
  }
}

function presentationSchema(placement) {
  const partIds = placement.parts.map(part => part.id);
  return {
    type: "object",
    additionalProperties: false,
    properties: {
      contractFingerprint: { type: "string", enum: [placement.fingerprint] },
      acknowledgedPartIds: { type: "array", minItems: partIds.length, maxItems: partIds.length, items: { type: "string", enum: partIds } },
      ...
      routingPresentation: { type: "object", additionalProperties: false, properties: { wireDiameterMm: { type: "number" }, minimumBendRadiusMm: { type: "number" }, style: { type: "string", enum: ["short-open-natural-arch"] }, loopsAllowed: { type: "boolean", enum: [false] } }, required: ["wireDiameterMm", "minimumBendRadiusMm", "style", "loopsAllowed"] }
    },
