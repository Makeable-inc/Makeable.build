...
function resolveSelectedParts(ids, idea) {
  ...
  for (const id of requested) {
    let part = catalogById.get(String(id));
    // Legacy preview plans do not compile an exact carrier/pin contract. The
    // SUUOO approval is restricted to the atomic full-size S3 carrier route,
    // not a blanket 5V exception for arbitrary controllers such as XIAO.
    if (part?.asin === "B0GX88M8ML") part = findPart("MTDELE");
...
function pruneUnrequestedControls(parts, idea, plan) {
  ...
  return parts.filter((part) => {
    // Integrated touch is incidental to a selected controller/display. Do not
    // remove the whole computer just because touch was not requested.
...
function fallbackPartsForIdea(idea) {
  ...
  if (/(knob|dial|rotary|encoder)/.test(text)) add("MTDELE");
...
function fallbackDisplayForIdea(idea) {
  ...
  if (!/(display|screen|status|dashboard|clock|timer|message|readout|interface|chart|graph|weather station|air quality|pet|companion|character|face|animation|game)/.test(text)) return null;
  if (/(tiny|wearable|badge|tag|single[- ]line|minimal status|micro status)/.test(text)) return findPart("0.91-inch I2C OLED");
  if (/(dashboard|chart|graph|touch|weather station|air quality|color trends|data panel)/.test(text)) return findPart("ESP32-C6 1.47inch IPS Touch Display");
  if (/(16x2|1602|character lcd|retro.*(?:clock|timer)|kitchen timer|countdown)/.test(text)) return findPart("16x2 / Pre-Soldered Pin Header") || findPart("B0F1L7FG5Z");
  if (/(pet|companion|character|face|animation|game|pixel art|color display)/.test(text)) return findPart("B0H2HL5ZQY");
  if (/(music|message|large monochrome|e[- ]?paper-like|high contrast)/.test(text)) return findPart("2.42 Inch OLED Display Module");
  return findPart("B0H6P19FQ7") || findPart("B0H2HL5ZQY") || findPart("2.42 Inch OLED Display Module");
}
...
function findPart(nameNeedle) {
  ...
  return catalog.find((part) => (
    ...
    && `${part.asin} ${part.name} ${part.listingTitle} ${part.subtype}`.toLowerCase().includes(needle)
  ));
}
  ...
  return [
    ...
    isPlantOrSoilBuild
      ? hasSoilMoistureSensor
        ? "Plant/soil architecture contract: Make this one compact self-contained instrument placed directly into soil. Integrate the controller head and the selected capacitive sensing blade into one continuous device. Keep the complete electronics head and service seam above the soil line, add a protective overhanging soil-stop shoulder, and extend the real selected sensing blade directly below it. Never show a separate tabletop station, detached probe, external harness, cable-connected sensor, decorative plant silhouette, or second device."
...
function renderDesignBrief(idea, plan, parts) {
  ...
  if (/pen holder|pencil holder|desk organizer/.test(text)) {
    ...
    signatureSilhouette = /plant|soil/.test(text)
      ? "a compact upright above-soil environmental stake with a readable interaction head; extend only an exact selected sensing blade below it, never an invented blade or separate probe"
      : /water/.test(text)
