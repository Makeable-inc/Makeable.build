...
export async function createBuild(body, options) {
  ...
  const resolvedParts = pruneUnrequestedControls(
  ...
  const parts = typeof options?.finalizeSelectedParts === "function"
    ? options.finalizeSelectedParts(resolvedParts, { idea, plan: selected })
    : resolvedParts;
  ...
  await options.onEvent?.("parallel_generation_started");
  ...
  const artifactsPromise = typeof options.generateArtifacts === "function"
    ? (async () => {
        await options.onEvent?.("artifact_generation_started");
        const artifacts = await options.generateArtifacts({
          idea,
          parts,
          plan: normalizedPlan,
          geometry,
  ...
  const [image, artifacts] = await Promise.all([imagePromise, artifactsPromise]);
...
function fallbackPartsForIdea(idea) {
  ...
  if (/\b(?:motion|presence|walk|person|occupancy)\b/.test(text)) add("LD2410C", "DIYables RGB LED Module");
