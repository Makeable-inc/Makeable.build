...
function normalizePlan(plan, idea, parts) {
  ...
  const copy = beginnerBuildCopy({ title: plan?.title || fallback.title, idea });
...
export function publicBuild(build) {
  const copy = beginnerBuildCopy(build);
  return {
    id: build.id,
...
function readableTitle(title, idea) {
  ...
  const words = source
    .replace(/[^a-z0-9\s-]/gi, " ")
    .split(/\s+/)
    .filter((word) => !new Set(["esp", "esp32", "arduino", "raspberry", "iot", "wifi", "bluetooth", "inspired", "style", "project", "module", "controller", "development", "board"]).has(word.toLowerCase()) && word.length > 1);
  const leadingFillers = new Set(["a", "an", "the", "make", "build", "create", "simple", "smart"]);
