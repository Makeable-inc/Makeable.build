...
export function parseSocialCsv(csv) {
  ...
  rows.slice(1).forEach((values, index) => {
    ...
    const record = {
      ...
      followersGained: metric(value("followers_gained"), "followers_gained", rowNumber),
      clicks: metric(value("clicks"), "clicks", rowNumber),
...
export async function readSocialRecords(store) {
  ...
  return mergeSocialRecords([], Array.isArray(stored?.records) ? stored.records : []);
}
...
function normalizeRecord(value) {
  ...
  return {
    ...
    followersGained: storedMetric(value.followersGained),
    clicks: storedMetric(value.clicks),
...
function summarize(records) {
  ...
  return {
    totalImpressions,
    totalEngagements,
    engagementRate: totalImpressions ? totalEngagements / totalImpressions : 0,
    followersGained: sum(records, "followersGained"),
    totalClicks: sum(records, "clicks"),
...
function accountSummary(records) {
  ...
  records.forEach((record) => {
    ...
    account.followersGained += record.followersGained;
    account.clicks += record.clicks;
    account.posts += 1;
    ...
    account.engagementRate = account.impressions
      ? account.engagements / account.impressions
      : 0;
