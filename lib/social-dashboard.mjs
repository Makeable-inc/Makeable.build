...
export function reconcileRefreshedSocialRecords(existing, incoming, refreshedPlatforms, failedPlatforms = []) {
  ...
  const failed = new Set(
    (Array.isArray(failedPlatforms) ? failedPlatforms : [])
      .map((platform) => boundedText(platform, 20).toLowerCase())
      .filter((platform) => PLATFORMS.has(platform)),
  );
  const retained = (Array.isArray(existing) ? existing : [])
    .filter((record) => !refreshed.has(boundedText(record?.platform, 20).toLowerCase()))
    .map((record) => failed.has(boundedText(record?.platform, 20).toLowerCase())
      && record?.coverage === "connected"
      ? { ...record, coverage: "stale" }
      : record);
